...
string[] moviesss = moviess.ToArray();

for (int i = 0; i < moviesss.Length; i++)
{
	if (movieNumber == 1)
	{
		int ticketPrice = 12;
		int totalPrice = ticketsCount * ticketPrice;
		Console.Write($"\nTotal Price: {totalPrice} ({ticketsCount} tickets * {ticketPrice} each)\n");
		Console.WriteLine();
		break;
	}
...
int[] ticketsCount2 = new int[ticketsCount];
...
for (int i = 0; i < ticketsCount2.Length; i++)
{
